...
def get_model_transfer_learning(model_name="resnet18", n_classes=50):
    ...
    if hasattr(models, model_name):
        ...
        netTransfer = getattr(models, model_name)(pretrained=True)
    ...
    # Loop over all parameters. If "param" is one parameter, "param.requires_grad = False" freezes it
    for param in netTransfer.parameters():
    ...
    num_ftrs = netTransfer.fc.in_features

    # Create a new linear layer with the appropriate number of inputs and outputs
    netTransfer.fc = nn.Linear(num_ftrs, n_classes)
