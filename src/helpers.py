...
def setup_env():
    ...
    if use_cuda:
        ...
        torch.cuda.empty_cache() # In case we ran on GPU before, need to free GPU memory
...
def get_data_location():
    ...
    """
    ret = ""
    # Get local current working directory, data folder should be in here
    data_folder = "data"
    subfolder = "landmark_images"
    folder_path = os.path.join(os.getcwd(), data_folder, subfolder)
    folder_path = os.path.normpath(folder_path)

    if os.path.exists(folder_path):
        ret = folder_path
    elif os.path.exists("/data/landmark_images"):
        ret = "/data/landmark_images"
    else:
